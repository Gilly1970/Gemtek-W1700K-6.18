...
function device_htmode_append(config) {
	...
	if (wildcard(config.htmode, 'EHT*')) {
		 ...
		 * Replaces the inert 010 quilt patch, which targeted the shell
		 * mac80211.sh that the ucode build discards. */
